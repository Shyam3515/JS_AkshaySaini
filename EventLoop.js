/** Whenever we try to execute a JS code a GEC is created and pushed onto the callStack */
...
2) DOM API's - The DOM API is a set of interfaces that we can use in JavaScript to manipulate the DOM.
...
These are all part of browsers not JS, and browser gives us access inside this JSCallStack or JSE,
*/
...
setTimeout(function cb() {
  ...
  //And the Job of this EventLoop is to check this callBackQueue and put the functions in callBackQueue inside the callStack.
...
//when you access this DomAPI, this DomAPI it basically access this HTML code and tries to find out a button with some ID and returns it.
document
  .getElementById("btn")
  //This addEventListener another super power given by the browser to the JSE, through the window object in the form of Web API 'which is Dom API'.
 ...
 * Fetch basically goes and requests a API call, and then returns a promise, and you have to pass a CBFunction which will be executed once this promise is resolved.
 */
console.log("Start");
...
//fetch is a webAPI which is used to make Network Calls. This netflix servers returns the data to fetch. Once we get data to here this CBF will now be ready this cbF will now be ready for the execution. They will be executed in 'MicroTaskQueue'.
